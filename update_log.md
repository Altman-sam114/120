...
- 最终修复提交 `051b790bbbcb8a114fe97158adc92785bec78b65` 的 Actions run `31595151881` / attempt 1 / job `94108775667` 已由 Agent C 使用 `Altman-sam114` 下载并核对 artifact `rustwar-ci-v1.2-main-051b790-run31595151881-attempt1` 到 `/private/tmp/rustwar-c-review-31595151881/`，目录约 1.7M。manifest 的 branch/SHA/run/attempt 完全匹配；JUnit 为 8 checks、0 failures、1 个既定 browser skip，`swift test` 为 331 tests passed，`git diff --check`、`node --check app.js`、Xcode list/build、双 Simulator 启动、横屏归一化、截图和两份 pixel probe 全部 success。Home / Combat PNG 均为 2622x1206，mean 分别为 `79.77828765459367` / `75.1291669470439`，std 分别为 `41.71567287350822` / `42.25349367470018`，range 均为 `255`；人工复看确认 Home 的 Production / Factory Tech 与队列首屏、Combat 的 5 个作战单位、Attack target pending HUD、弹道、爆点和小地图均正常。v2.49 实现通过该云端 artifact 验收。
...
- battlefield hint/status 的 outer frame 在自然换行内容后使用垂直 intrinsic sizing 与 layout priority，避免 compact trailing header 中的 target 标题、详情最后一行或多行错误状态脱离边框；保留 44pt 下限、黄色 pending 外框、VoiceOver 和既有 Dynamic Type 语义。
- Tactical Map pending Attack 将约 16pt 屏幕直径换算为 world minimum hit radius，沿既有可见性过滤、最近目标排序和 Attack command path 命中敌方 marker；普通 map camera tap 与其它 pending command 不改变。
- 只修改 iOS HUD/map presentation 与 hit-area 参数传递；Core、GameStateSelection、命令结果、TouchSequenceOwner、战斗、AI、存档/JSON 和 Web 版不变。参考 Rusted Warfare 仍只用于战场主体、直白 pending feedback 和可点击目标层级的信息架构，不复制素材。
...
- `md/flow/flowchart.md`
- `md/test/test.md`
- `md/prompt/v1-ios-swift-port/v2.55-target-feedback-and-map-hit-areas.md`

验证状态：

- 按云端唯一验证制度未运行本地 SwiftPM test、Swift typecheck、Xcode build/list、Simulator、Preview、浏览器、截图或 `git diff --check`；只读源码与云端结果包用于范围审查。
- 实现提交 `0b78d8529e8809d4828c4a55d340a406b3d7519e` 已 push 至 `origin/main`；GitHub Actions run `32260424962`、attempt `1`、job `96092104045` 成功，未加密 artifact `rustwar-ci-v1.2-main-0b78d85-run32260424962-attempt1`（artifact id `9368383533`）已下载到 `/private/tmp/rustwar-c-review-32260424962/`，目录约 `1.7M`。
- manifest 的 `branch=main`、`commitSha=0b78d8529e8809d4828c4a55d340a406b3d7519e`、run id 和 run attempt 与 `origin/main` 完全一致；JUnit 为 `8 tests / 0 failures / 1 expected browser skip`，Swift Core `340 tests passed`，Xcode `26.5`、iOS SDK `26.5`、Swift `6.3.2`、iPhone 17 Pro，双架构 build、production/combat 启动、横屏归一化和双 pixel probe 全部成功。
- Home PNG 为 `2622x1206`、SHA-256 `c90459c86aa225ee2fb0dc0b9cc4c33908e0c339bd7db1202258c8c33b12b12d`；Combat PNG 为 `2622x1206`、SHA-256 `2a86e468fa29d13efc9802d0c24e6f286b903e561e20e942ed0abe241cab80ab`。人工复看确认 Attack target 多行提示完整包含于边框内，Home 生产区、Tactical Map 与战斗模型无明显回退；唯一 annotation 为 `actions/upload-artifact@v5` 的 Node 20 弃用提示，不影响 gate，v2.55 通过云端 artifact 验收。
...
- 实现 commit `4029337830fe5cb906ce97597c795936e735e6b7` 已推送到 `origin/main`。Actions run `32643241133` / attempt `1` / job `97203471829` 成功；Agent C 已把 artifact `rustwar-ci-v1.2-main-4029337-run32643241133-attempt1` 下载到 `/private/tmp/rustwar-c-review-32643241133/`，目录约 `1.7M`。
- manifest 的 branch、完整 SHA、run/attempt、Xcode 26.5、iOS 26.5、Swift 6.3.2 与固定 iPhone 17 Pro 完全匹配。JUnit `8 tests / 0 failures / 1 skipped`，Core `342 tests`，`BattlefieldScene.swift` 进入 arm64/x86_64 编译；静态检查、Xcode list/build、production/combat 双启动、横屏归一化和双 PNG probe 全成功。唯一 skip 为既有 headless browser regression；唯一 Actions 注释为 `upload-artifact@v5` 的 Node 20 被强制按 Node 24 运行，不影响结果。
- `ios-home.png` 为 `2622x1206`、透明比例 0、SHA-256 `f2238e3bfb7918b0db80f2cda7d97533c670db51e7c91c358a3a24acb72a1bcf`，与 v2.74 逐字节一致。`ios-combat.png` 为 `2622x1206`、透明比例 0、SHA-256 `01fcba16813a13a14b93dcbd78f13573ae2ab29a98327768af91af5bda0bab9e`；人工确认三个 terminal + impact 重叠目标的 hull、炮塔/发射器、朝向和阵营标记清楚，边缘特效仍可辨，v2.72 武器层级、双方 Turret、Quick Orders、Tactical Map 和状态栏无静态回退。Agent C 判定 v2.75 实现 artifact 验收通过。
...
- 实现 commit `243723bc24aa161cda46a17c50cfb41294b7a0d6` 已推送到 `origin/main`，对应 Actions run `33296047909` / attempt `1` / job `99215769992` 成功；artifact `rustwar-ci-v1.2-main-243723b-run33296047909-attempt1`（ID `9727507874`，GitHub digest `sha256:da60a08ef50d995983f5361d3015aa8a08f06f98b6fa61673f2afc3d9dbed392`，网页大小 1.32 MB，下载目录约 1.7M）已由 Agent C 下载到 `/private/tmp/rustwar-c-review-33296047909/` 并核对。
- manifest 的 `branch=main`、完整 SHA、run/attempt、固定 Xcode 26.5 / iOS 26.5 / Swift 6.3.2 / iPhone 17 Pro 完全匹配。JUnit `8 tests / 0 failures / 1 skipped`、Core `344 tests`、SwiftUI 修改文件双架构编译、静态检查、Xcode list/build、production/combat 双场景启动、横屏归一化和双 PNG probe 全成功；唯一 skip 为既有 headless-browser regression。
- `ios-home.png` 为 `2622x1206`、透明比例 0、SHA-256 `8ca15f5341017c1760863c38d64c70bb5b86c789907cd2371c44ce2e5ff2513f`，与 v2.81 一致；`ios-combat.png` 为 `2622x1206`、透明比例 0、SHA-256 `b6eb46da36d6363de916dce479edb7de4eaa6f01b1c321ba006423656e975021`。Agent C 人工确认普通 compact 的短 hint 使 Quick Orders / Commands 更早可见，Home 生产 HUD、Combat 模型、武器层、selection/HP、v2.78 marker、terminal/impact、Tactical Map 和状态栏无静态回退，v2.82 artifact 验收通过。
