...
- v1.90/v2.33 地形回归应分别进入 Coast、Islands、Lava：确认 grass/grass2/dirt/sand/rock/water/deep/lava 维持可辨识色阶，water/deep 同类连续区域不再出现逐格棋盘式明暗，跨格长波纹保持在水域内部，海岸泡沫连续，water/deep 分界克制，熔岩焦岸和亮裂隙清楚，正常相机 zoom 没有明显 tile 裂缝。单位、建筑、选择环、HP/进度条、命令线和雷达信号必须继续比地表细节醒目，浅雾/深雾仍覆盖全部材质层。
- v1.90/v2.33 代码验收还要确认 `drawTerrain` 的非水基础节点按 3 色阶聚合、water/deep 固定统一基底，高光与波峰按连续水域 run 写入两个 compound path，细节/边界节点数量固定，不为每个 tile 或 run 新增节点；稳定 hash 不使用随机数、时间或 `Hasher`，相邻边界只检查右/下且先验证 bounds，地图外不产生 grass fallback 假海岸。地图切换、Restart、Load 才通过既有 map revision 路径重建地形，普通 `renderNow()` 不重复生成路径。
