...
- v1.90/v2.33-v2.35 地形回归应分别进入 Coast、Islands、Lava：确认每种地形保持可辨识，`grass` / `grass2` 不再形成绿色棋盘，非水三档逐 tile 明暗消失，grass-family/dirt/sand/rock 的软纹与细纹可跨连续 tile 且不漏入其它材质；water/deep 不恢复棋盘或 hairline，海岸、深度和熔岩边界保持。单位、建筑、选择环、HP/进度条、命令线和雷达信号必须继续比地表细节醒目，雾层仍覆盖全部材质。
- v1.90/v2.33-v2.35 代码验收还要确认基础节点为每种 `TerrainKind` 一个 compound path，`grass` / `grass2` 只在表现层映射为同一 surface family，基础 node 继续使用同 fill 色 covering stroke；land soft/fine path 按 family 聚合，不为 tile 或 run 新增 node。稳定 hash 不使用随机数、时间或 `Hasher`，地图切换、Restart、Load 才重建地形，普通 `renderNow()` 不重复生成路径。
