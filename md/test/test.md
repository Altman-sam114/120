...
- v2.19 云端验收必须确认 `BattlefieldScene.swift` 双架构编译，普通 impact 仍只由当前可见单位/建筑 HP 单次下降触发；每次 impact 只新增一个短寿命焦坑和一个有界 effect container，焦坑必须含椭圆外缘、内坑、低透明余烬 rim 与确定性放射裂纹，爆炸必须含贴地 bloom、两层不同齿数/旋转的 corona、既有爆心/火球/冲击环、火花、碎片和三团烟。动态模式不得新增 timer/Task/常驻 action；Reduce Motion 不得移动、旋转或扩张；fog 下层、map reset、64 effect / 32 decal 淘汰和 Core/JSON/战斗数值保持。combat fixture 复用同一 frozen impact，并只追加一个位于阵型中央且不遮挡单位/HUD的明确落弹点；Agent C 人工确认 `ios-home.png` 不变，`ios-combat.png` 至少一处可辨双冠火焰、贴地冲击波、烟尘和裂纹焦坑，同时保留双方 Turret、单位后坐、弹道与模型可读性。静态图不能证明动画曲线、动态 Reduce Motion、持续密集战斗性能或真机观感。
