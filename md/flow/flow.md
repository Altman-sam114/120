...
- v1.52 起，`GameState.controlGroups` 保存 1-9 号控制编队 ID 快照，`GameEngine.storeControlGroup(_:)` 保存当前有效己方选择，`GameEngine.recallControlGroup(_:)` 惰性过滤已死亡、缺失或非己方实体并写回多选集合；iOS HUD 暴露 1-3 号紧凑 Save / Recall 按钮，Save/Load 随 `GameState` 保存恢复编队。
