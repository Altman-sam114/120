...
v2.36 将相邻不同 land surface family、water/deep、water/land 与 lava/non-lava 的表现边界统一交给 `appendOrganicBoundary`：每条原 tile edge 保持端点相接，但两个控制点沿法向做不超过 3.4 world pt 的稳定偏移。land boundary 按 grass < dirt < sand < rock 优先级选择覆盖材质，用 10.5pt 同材质底带遮住原直线接缝，再叠加 1.15pt 低对比边缘；海岸、depth 和 lava bank 同样先用足宽底带覆盖接缝，再绘制原有细边。所有 segment 仍聚合进按材质/边界类型固定的 compound path，仅在地图 revision 重建，不改变 Core tile 或交互语义。
...
- v2.36 起，上述相邻边界不再直接 `addLine`，而是以原 edge 端点和稳定法向偏移生成三次曲线。不同陆地 family 先按固定优先级选出覆盖材质并聚合到该 family 的 boundary path，10.5pt 同色 stroke 覆盖直角接缝，细 accent 描出有机轮廓；coast/depth/lava bank 也使用足宽底带覆盖原接缝。最大 3.4pt 偏移小于底带半径，端点保持原网格顶点以避免 segment 断开；路径节点数仍按 family/type 固定，只有路径段数量随真实边界增长。
