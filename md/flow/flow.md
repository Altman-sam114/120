...
- v2.22 起，production fixture 保持 T1 Land Factory、1050 metal 和既有四项队列，因此 Factory Tech 首屏必须显示可用的 `Upgrade T2 - 900 Metal`、`1.25x production | 1200 HP`，同时保留完整 Build Queue；普通启动仍不预选、不预置队列。公开视频检索实际观看到 “Rusted Warfare Guide Units and Factories” 0:02 帧中的右侧紧凑生产网格、右上小地图、顶部资源与中央选中生产建筑，但该帧没有可见升级按钮，因此只作为高密度建筑操作布局参考，不作为具体升级外观证据。
...
- v1.28 起，原生 Land Factory 的 T1 生产列表扩展为 Scout / Light Tank / Hover Tank / Artillery / AA Tank，并按 Web T1 顺序驱动 `queueUnit`、Cancel Production、Repeat 和 Rally；iOS HUD 生产按钮区改用自适应网格，避免五个生产按钮在窄屏单行挤压。该轮尚未迁移的 Land Factory T2 已由 v2.22 补入生产倍率/HP/vision 升级；重型单位、维修车、护盾车或其它生产建筑仍未迁移。
