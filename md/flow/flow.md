...
v1.90 新增 iOS 程序化地形材质：`BattlefieldScene` 只读既有 `TerrainGrid`，用稳定整数 hash 把非水地形分成 3 档色差并聚合为 compound fill path，再聚合草痕、砂土/岩石细节、水面/熔岩纹理和海岸/深水/熔岩边界。v2.33 起 `water` / `deep` 各自改用统一基底，并按每行连续水域 run 聚合跨格高光和长波纹，消除逐格明暗拼贴。地形路径只在 map id 或 `mapRenderRevision` 变化时重建，节点数由地形/色阶/细节类别决定，不随 tile 数量线性增长，也不回写 Core。
...
- v1.90 起，`BattlefieldScene.drawTerrain` 不再给每个 44pt tile 创建独立节点。Scene 用稳定整数 hash 为非水 `TerrainKind` 选择 3 档轻微色差，water/deep 自 v2.33 起固定为各自统一基底；基础矩形仍聚合进 compound `CGMutablePath`，v2.34 起用同填充色 1pt stroke 覆盖 compound 子路径的像素 hairline。草地、砂土、岩石和熔岩细节各自聚合，连续水域的高光/长波纹按 horizontal run 聚合为两个固定 path node。相邻边界只检查右侧和下侧且先做 bounds 门控，水域/陆地生成岸脚与泡沫，water/deep 生成深度线，lava/非 lava 生成焦岸与热边，避免重复线段和地图外 grass fallback 假边界。节点数不随 tile/run 数线性增长，只在既有地图重建路径运行；`terrainNode` 仍位于资源、实体、`effectNode`、`fogNode` 和雷达层下方，Core 地形、通行、雾和存档不变。
