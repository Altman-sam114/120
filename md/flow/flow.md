...
v1.92 修正 iOS 短高度横屏 HUD 分类：`TacticalHUDLayoutRole` 先判断横屏且高度低于 520pt，再判断 regular 宽度，避免 844x390 / 874x402 手机因宽度超过 700pt 被误判为 iPad regular trailing。compact trailing dock 改为容器宽度 30%、clamp 224-260pt，并继续驱动单列命令网格和 120x80 Tactical Map；真实 iPhone 17 Pro Simulator 前后截图确认战场变宽且长标题完整。
