...
RustwarCore MapPreset / GameState / GameEngine
  ...
  -> CameraState / UserDefaults save payload / pause-speed gate / GameEngine.select / GameEngine.issueMove / GameEngine.issueAttackMove / GameEngine.issuePatrol / GameEngine.issueStop / GameEngine.issueAttack / GameEngine.queueUnit / GameEngine.cancelLastProduction / GameEngine.setRally / GameEngine.update / GameEngine(state:)
...
- 计算收入、人口、简单 tick、实体命中选择、己方单单位移动、己方单单位 Attack-Move、己方单单位 Patrol、己方 Stop、己方单单位攻击、基础伤害/死亡清理、陆军工厂生产队列、生产取消/退款、己方工厂集结点设置、红方最小生产/进攻 AI，以及从已保存 `GameState` 恢复 `GameEngine`。
...
- 支持 Coast / Islands / Lava 地图切换、当前地图重开、tap 选择、Move / Attack Move / Patrol 模式落点、拖拽平移、捏合缩放、暂停/恢复、0.5x / 1x / 2x 速度切换和基础 economy tick。
...
- 改 `swift/RustwarCore/`：本地尽量跑 `swift test --package-path swift/RustwarCore`；若本机 SwiftPM 阻塞，至少尝试 `swiftc -typecheck swift/RustwarCore/Sources/RustwarCore/*.swift` 并记录工具链错误；当前 Swift tests 覆盖初始化、经济 tick、选择、移动、Attack-Move、Patrol、Stop、生产、生产取消/退款、工厂集结点、基础攻击/死亡清理、红方生产/进攻 AI、`GameState` JSON 往返和恢复后继续模拟。
