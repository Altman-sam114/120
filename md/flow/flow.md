...
RustwarCore MapPreset / GameState / GameEngine
  ...
  -> CameraState / UserDefaults save payload / pause-speed gate / GameEngine.select / GameEngine.issueMove / GameEngine.issueAttackMove / GameEngine.issueStop / GameEngine.issueAttack / GameEngine.queueUnit / GameEngine.cancelLastProduction / GameEngine.setRally / GameEngine.update / GameEngine(state:)
...
- 计算收入、人口、简单 tick、实体命中选择、己方单单位移动、己方单单位 Attack-Move、己方 Stop、己方单单位攻击、基础伤害/死亡清理、陆军工厂生产队列、生产取消/退款、己方工厂集结点设置、红方最小生产/进攻 AI，以及从已保存 `GameState` 恢复 `GameEngine`。
...
- 支持 Coast / Islands / Lava 地图切换、当前地图重开、tap 选择、Move / Attack Move 模式落点、拖拽平移、捏合缩放、暂停/恢复、0.5x / 1x / 2x 速度切换和基础 economy tick。
...
- v1.6 起，`GameController.currentMapID` 由 HUD Map picker 绑定，切换地图或 Restart 会重建 `GameEngine(mapID:)`、重置 `CameraState`、清除待选 Move/Attack/Attack Move 模式，并推进 `mapRenderRevision` 让 `BattlefieldScene` 在同图重开时也刷新地形和资源层。
...
- v1.8 起，选中己方单位时 HUD 显示 Stop 命令；点按 Stop 会调用 `GameEngine.issueStop()` 清除当前选中玩家单位的 `UnitSnapshot.order`，并由 `GameController` 取消待选 Move/Attack Move/Attack 目标模式。SpriteKit 订单线会随 `order == nil` 自然消失。
...
- v1.11 起，HUD 显示 Save / Load 按钮；`GameController.saveGame()` 用 `JSONEncoder` 把 app-private save payload 写入 `UserDefaults` 单槽，payload 包含 schema version、`GameState`、`CameraState`、当前 `MapID`、暂停状态、速度和 AI 开关；`loadGame()` 解码后用 `GameEngine(state:enemyAIEnabled:)` 恢复 core 状态，恢复相机、地图、暂停和速度，清空待选 Move/Attack Move/Attack/Rally 模式，并递增 `mapRenderRevision` / `renderRevision` 让 SpriteKit 和 SwiftUI 刷新。
...
- 改 `swift/RustwarCore/`：本地尽量跑 `swift test --package-path swift/RustwarCore`；若本机 SwiftPM 阻塞，至少尝试 `swiftc -typecheck swift/RustwarCore/Sources/RustwarCore/*.swift` 并记录工具链错误；当前 Swift tests 覆盖初始化、经济 tick、选择、移动、Attack-Move、Stop、生产、生产取消/退款、工厂集结点、基础攻击/死亡清理、红方生产/进攻 AI、`GameState` JSON 往返和恢复后继续模拟。
