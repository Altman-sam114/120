...
- 计算收入、人口、简单 tick、实体/残骸/资源点命中选择、己方单单位和多单位移动、己方单单位和多单位 Attack-Move、己方单单位和多单位 Patrol、己方单单位和多单位 Guard、己方单 Builder Repair、己方单 Builder Reclaim、己方单 Builder 建造 Extractor、己方单 Builder 建造 Turret、己方单 Builder 建造 Land Factory、己方单单位和多单位 Stop、己方单单位攻击、炮塔自动防御开火、基础伤害/死亡残骸清理、Command Center Builder 生产、Land Factory T1 生产队列、生产取消/退款、重复生产、己方生产建筑集结点设置、红方最小生产/资源扩张/维修/回收/Land Factory 建造/Turret 建造/进攻 AI、红方 AI Web-lite 目标评分，以及从已保存 `GameState` 恢复 `GameEngine`。
...
- 支持 Coast / Islands / Lava 地图切换、当前地图重开、tap 选择、Move / Attack Move / Patrol 模式落点、Guard 友方目标点选、Repair 受损友方目标点选、Reclaim 残骸目标点选、Build Extractor 资源点目标点选、Idle Builders / Combat Units 批量选择、多单位 Move / Attack Move / Patrol / Guard / Stop、拖拽平移、捏合缩放、暂停/恢复、0.5x / 1x / 2x 速度切换和基础 economy tick。
...
- v1.22 起，战术小地图在 Attack / Guard / Repair 等待态下会调用现有 `GameState.selectionTarget(at:includeEnemies:)` 命中单位或建筑，并复用 `issueAttack` / `issueGuard` / `issueRepair` 的目标合法性校验；未命中或目标非法时沿用主战场相同状态文案。v1.42 后，Guard 小地图实体目标命令自然复用 core 的多选集合语义。
