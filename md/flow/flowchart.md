...
读图说明：这张图描述后续开发管理流程。人工先给目标，Agent A 只写实现提示词，Agent B 负责代码和测试，Agent C 负责验收和更新核心逻辑文档，最后回到人工复核。
...
flowchart TD
  ...
  B1 --> C1["Agent C: 验收与核心文档更新<br/>中文注释：查 diff、核测试、判断是否满足目标"]
  C1 --> F["md/flow + update_log + md/test<br/>中文注释：记录真实架构、重要历史和测试基线变化"]
  F --> H2["人工复核<br/>中文注释：确认结果或提出下一轮目标"]
  H2 --> H
```
