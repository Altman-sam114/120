...
flowchart TD
  ...
  Q --> E
  C --> UPG["BuildingUpgradeResult / BuildingUpgradeCancelResult / BuildingSnapshot.upgradeLevel / upgradeProgress<br/>中文注释：选中完成状态己方 Radar Station 或 Extractor 且金属足够时，Upgrade 扣除金属并启动 T2 进度；Cancel Upgrade 可取消当前 T2 进度并按剩余进度退款；GameEngine.update 推进升级，Radar T2 提高 HP 上限、vision 和 radarRange，Extractor T2 提高 HP 上限、income 和 vision"]
  UPG --> E
