...
v2.82（代码已完成，等待云端验收）：原生 iOS 普通 compact command dock 的 Battlefield interaction hint 现在使用按状态派生的短可见文案，让“点敌人攻击 / 点空地 Attack-Move / 拖动框选 / 点位置”不再用长句把 Quick Orders 推到首屏之外；VoiceOver、regular/accessibility Dynamic Type 继续读取完整触控说明，生产建筑、命令 action、44pt、Core、战斗和 Web 版不变。本轮借鉴 Rusted Warfare 对快速发令、multi-touch、unit groups 和 strategic zoom 的公开产品定位，属于 SwiftUI presentation-only 改动，最终 commit、Actions run、artifact 和 PNG hash 待补。
