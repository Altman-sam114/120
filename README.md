...
- v1.70 起，原生 core 会按己方存活单位和完成建筑计算玩家当前可见 tile，iOS 主战场会用暗色雾层覆盖当前不可见 tile；本轮尚不隐藏敌方实体，也不包含已探索记忆或雷达信号。
...
- Visibility：主战场会根据当前存活己方单位和完成己方建筑的视野，用暗色雾层覆盖不可见 tile。当前版本仍会绘制敌方实体和战术小地图目标，用于保持调试和迁移可读性。
