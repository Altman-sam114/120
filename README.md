...
v2.80（待云端验收）：原生 iOS compact producer dock 的固定 header 已显示 `Production / Land Factory / T2 / 1.25x`，因此普通非辅助功能字号下不再在 Production section 紧接着重复渲染第二个 `Production` section title，management rail、NOW / QUEUE / UPGRADE、Factory Tech、生产卡和队列更早进入首屏。regular、compact 下的辅助功能 Dynamic Type、无 producer focus 的 section、VoiceOver、44pt、快捷键、生产 action、Core、存档和 Web 版保持；本轮只改变 SwiftUI presentation gate，最终以最新 `origin/main` 对应 Actions artifact 验收为准。
