...
v2.81（待云端验收）：原生 iOS 单位主体的中间装甲面增加低饱和队伍色偏移：玩家为绿灰、敌方为红灰；原有深色钢灰层级、八类单位的几何模型、炮塔/发射器、v2.78 hull 尾缘单/双阵营标记、选择/HP/战斗特效保持。该轮只改变 `BattlefieldScene.unitBody` 的 presentation，不改变建筑、武器后坐、Core、命令、输入、生产、AI、存档、HUD、Tactical Map 或 Web 版；云端 artifact 验收完成后补充 commit、run、artifact 与 PNG 哈希。
