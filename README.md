...
v2.83（代码已完成，等待云端验收）：原生 iOS 主战场把单指 `tap / preview / pan / Select Area` 收敛到同一个零距离 `DragGesture` 路由；达到既有 12pt policy 后立即由同一 callback claim pan/area lease，避免独立 `SpatialTapGesture` 与第二个 12pt drag 的 callback 竞态造成拖动死区。双指框选/捏合、长按上下文、TouchSequenceOwner、命令语义、44pt、生产、模型、战斗和 Web 版保持；这是 BattlefieldView 输入层改动，最终 commit、Actions run、artifact 和 PNG hash 待补。真实设备 callback 顺序与手感仍需实机证据。
