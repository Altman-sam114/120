...
- Upgrade Extractor：选中完成状态己方 Extractor 且金属足够时显示；点按后消耗 650 金属并启动 20 秒 T2 升级，完成后收入提升到 18、HP 上限提升到 760、真实视野提升到 290。升级进度会显示在建筑下方，升级中可用 Cancel Upgrade 按剩余进度退款。
...
- Cancel Upgrade：选中正在 T2 升级的完成状态己方 Radar Station 时显示；点按后取消当前升级进度，并按剩余进度返还金属。
