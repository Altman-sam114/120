...
- `ios/RustwarIOS/`：原生 SwiftUI + SpriteKit iOS App，启动后从 `RustwarCore` 状态显示战场地形、资源点、双方初始建筑/单位、战斗残骸和 HUD；支持 Coast / Islands / Lava 地图切换、重开当前地图、tap 选择、拖拽平移、捏合缩放、右下战术小地图点按居中，或在 Move / Attack Move / Patrol / Rally 等待状态下下达点位命令，以及在 Reclaim / Build Extractor 等待状态下点选残骸或资源点目标、Pause/Play 和 0.5x / 1x / 2x 速度切换，选中己方单位后可用 Move 下达单单位移动命令、用 Attack Move 指定行军攻击目的地、用 Patrol 设置当前位置和端点之间的往返巡逻、用 Guard 点选友方单位或建筑进行护航、选中己方 Builder 时可用 Repair 点选受损友方单位或建筑进行维修、用 Reclaim 点选残骸持续回收金属、用 Build Extractor 点选空闲资源点扣金属并建造未完成采集器、用 Attack 点选敌方单位或建筑并显示血条/攻击目标、用 Stop 清除当前移动、攻击移动、巡逻、护航、维修、回收、建造或攻击命令，选中己方陆军工厂后可生产 Scout / Light Tank、取消队尾生产并按未完成进度退款，并用 Rally 改变后续出兵集结点；Save / Load 可用本机单槽存档保存和恢复当前原生对局、相机、地图、暂停和速度；完成状态炮塔会自动攻击射程内敌方单位并显示轻量火力线；红方会用空闲 Builder 扩张空闲资源点、从现有陆军工厂排队造兵并主动攻击玩家目标，并通过简单 economy tick 推进金属收入、建造、生产进度和基础战斗。
...
- Tactical map：右下小地图显示资源点、战斗残骸、双方单位/建筑和当前相机中心；无待选命令时点按小地图会把主战场相机居中到对应位置；Move / Attack Move / Patrol / Rally 等待落点时点按小地图会直接下达对应点位命令；Reclaim / Build Extractor 等待目标时点按小地图会尝试命中残骸或资源点并下达 Builder 命令。
- Reset：重置战场相机。
