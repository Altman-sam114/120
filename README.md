...
v2.79（待云端验收）：原生 iOS 主战场增加 fresh-touch handoff 的 terminal barrier。当同一 Spatial frame 同时出现新 active touch 与未被当前 owner quarantine 的 ended/cancelled ID 时，不立即播种新 touch sequence，等待无歧义 active frame，避免迟到旧 callback 污染点按、移动、Attack-Move 或多指框选。已 quarantine 的旧 ID 仍可正常交接；TouchSequenceOwner、BattlefieldView、Core 单测和输入语义保持，不声称能绝对区分所有 SwiftUI 旧触点回调；最终只认最新 `origin/main` SHA 的 GitHub Actions artifact。
