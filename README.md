...
v2.84（实现待云端验收）：原生 iOS 主战场新增独立的 `groundImpactNode`，把陆地/水面受击 container 放到 decal 与单位实体之间；单位 hull、炮塔/发射器、selection、HP 和阵营标记因此保持可读。炮口、弹道终点、terminal、指令确认与死亡爆炸仍留在前景 `effectNode`。两层 bounded effect 共用 64 个容量并按创建顺序 oldest-first 淘汰，32 个 scorch decal 上限、Reduce Motion、frozen combat fixture、Core、命令、生产、触控、存档和 Web 版保持。本轮只改变 `BattlefieldScene` 的 SpriteKit presentation 与效果生命周期管理，待最新 `origin/main` Actions artifact 复判后补充 run、artifact 和 PNG 哈希。
