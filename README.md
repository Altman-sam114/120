...
v2.86（本轮实现，等待云端验收）：原生 iOS 紧凑横屏 command dock 的二级 Commands 网格在普通字号下至少使用两列，让 `Select Area` 与 `Same Type` 不再因单列高度被截断；compact trailing 的这两个高频选择按钮改用图标上置的两行短标题，避免窄卡按字符断词，辅助功能 Dynamic Type 仍使用一列完整标签。紧凑三列生产卡把成本/人口与建造时间分成两行，Heavy 等长数字不再挤进一行；生产 action、不可用状态、Shift+1-9、44pt 和 VoiceOver 语义保持。主战场触摸序列新增 seed 时的 `battlefieldCameraRevision` lease：Tactical Map、键盘、Reset 或 Focus 在手指抬起前改变相机时，未 claim 的旧 preview 会立即清理，Area Selection 也会取消，旧 tap/long press 会安全失效；当前手势自己的 pan/pinch 仍可正常更新相机。Core、命令结果、生产数值、模型、战斗效果、存档/JSON 和 Web 版不变。本轮实现尚未以云端 artifact 宣布通过；`.wp` 继续保持未跟踪。
