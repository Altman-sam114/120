...
v2.86（云端验收通过）：原生 iOS 紧凑横屏 command dock 的二级 Commands 网格在普通字号下至少使用两列，让 `Select Area` 与 `Same Type` 不再因单列高度被截断；compact trailing 的这两个高频选择按钮改用图标上置的两行短标题，避免窄卡按字符断词，辅助功能 Dynamic Type 仍使用一列完整标签。紧凑三列生产卡把成本/人口与建造时间分成两行，Heavy 等长数字不再挤进一行；生产 action、不可用状态、Shift+1-9、44pt 和 VoiceOver 语义保持。主战场触摸序列新增 seed 时的 `battlefieldCameraRevision` lease：Tactical Map、键盘、Reset 或 Focus 在手指抬起前改变相机时，未 claim 的旧 preview 会立即清理，Area Selection 也会取消，旧 tap/long press 会安全失效；当前手势自己的 pan/pinch 仍可正常更新相机。Core、命令结果、生产数值、模型、战斗效果、存档/JSON 和 Web 版不变。最终修复 commit `bd437d2b07a928b8722d40715b3242bfc89dac98` 对应 Actions run `33487734249` / attempt `1` / job `99791650154` 的 artifact `rustwar-ci-v1.2-main-bd437d2-run33487734249-attempt1`（ID `9792827895`，digest `sha256:da958b3274976d55d24e47763b6254b4ad87da10c35f577783e9f511f87ebd0e`，下载目录 `/private/tmp/rustwar-c-review-33487734249/`）已按 Agent C 流程下载并核对；manifest 完整匹配，JUnit `8/0/1`、Core `344 tests`、Xcode 26.5 / iOS 26.5 / Swift 6.3.2 / iPhone 17 Pro、双架构编译、双场景启动、横屏归一化和双 PNG probe 全成功。Home `2622x1206` SHA-256 `9e29718ec238f2bc00e90e4ba7c0362687209275d990ac929ad7a66db99b1c60`，Combat `ed9b607cb12914e84feede51575d761f016e4875d63fa4199c0dea63bf37b14f`；既有 headless-browser regression 为唯一 skip。`.wp` 继续保持未跟踪。

v2.85（云端验收通过）：原生 iOS 主战场和 Tactical Map 共用 `battlefieldInputEpoch` 作为触控生命周期边界。切换 Replace/Add、进入或取消等待命令、视口尺寸变化和战场离开都会使当前 gesture sequence 失效；`BattlefieldView` 与 `TacticalMapView` 的 callback、preview、pan、框选、pinch、长按和最终提交都必须通过 seed 时的 epoch 检查，避免旧手指回调落入新的 pending command 或新视口。主战场的 pending touch preview 还复用正式 command confirmation 几何：Move、Attack-Move、Attack、Rally、Guard、Repair、Build、Reclaim 和 Select/Invalid 各有可读符号，不再只依赖颜色。命令语义、44pt 命中、TouchSequenceOwner、双指框选、生产、Core、存档、战斗数值、HUD、模型主体和 Web 版保持。实现 commit `9d209b83832450277a1d95e0ac83a16aee13123a` 对应 Actions run `33481289474` / attempt `1` / job `99771261711` 的 artifact `rustwar-ci-v1.2-main-9d209b8-run33481289474-attempt1`（ID `9790353160`，digest `sha256:7b5454df593eb71e2c72460116ff401bd0da5cee669701812d2c472d01ca544a`，下载目录 `/private/tmp/rustwar-c-review-33481289474/`）已由 Agent C 下载并核对；JUnit `8/0/1`、Core `344 tests`、四个 iOS 修改文件双架构编译、Xcode list/build、production/combat 双场景启动、横屏归一化和双 PNG probe 全成功。Home SHA-256 `8ca15f5341017c1760863c38d64c70bb5b86c789907cd2371c44ce2e5ff2513f`，Combat `723f2460d19a4379d84b3a567fe6a21ce699da593bfc7de2676aba56a9425022`；静态复看确认本轮没有 HUD、模型、武器、selection/HP、阵营标记、terminal/impact、Tactical Map 或状态栏回退。固定画面仍暴露既有 Heavy 生产卡指标截断和底部 Commands 滚动发现性问题，列入下一轮 UI 修复；真机 callback 时序与触控手感仍需后续证据。

v2.84（云端验收通过）：原生 iOS 主战场新增独立的 `groundImpactNode`，把陆地/水面受击 container 放到 decal 与单位实体之间；单位 hull、炮塔/发射器、selection、HP 和阵营标记因此保持可读。炮口、弹道终点、terminal、指令确认与死亡爆炸仍留在前景 `effectNode`。两层 bounded effect 共用 64 个容量并按创建顺序 oldest-first 淘汰，32 个 scorch decal 上限、Reduce Motion、frozen combat fixture、Core、命令、生产、触控、存档和 Web 版保持。本轮只改变 `BattlefieldScene` 的 SpriteKit presentation 与效果生命周期管理。实现 commit `33e7a420136945c80e3f6697e5ca6b0e21c7e13c` 对应 Actions run `33299697563` / attempt `1` / job `99225356862` 的 artifact `rustwar-ci-v1.2-main-33e7a42-run33299697563-attempt1`（ID `9728651408`，GitHub API 未提供 digest，网页大小约 1.32 MB，下载目录约 1.7M）已由 Agent C 下载到 `/private/tmp/rustwar-c-review-33299697563/` 并核对；manifest 与完整 SHA、branch、run/attempt 和固定工具链完全匹配。JUnit `8/0/1`、Core `344 tests`、`BattlefieldScene.swift` 双架构编译、Xcode list/build、production/combat 双场景启动、横屏归一化和双 PNG probe 全成功；既有 headless-browser regression 仍为唯一 skip。Home SHA-256 `8ca15f5341017c1760863c38d64c70bb5b86c789907cd2371c44ce2e5ff2513f`，Combat `723f2460d19a4379d84b3a567fe6a21ce699da593bfc7de2676aba56a9425022`。Agent C 人工复判确认地面受击不再盖住实体关键轮廓，前景 terminal/tracer/death 与 Home 生产 HUD、Tactical Map、状态栏无静态回退；动态时序、真机性能和触控手感仍属于证据边界。

v2.83（云端验收通过）：原生 iOS 主战场把单指 `tap / preview / pan / Select Area` 收敛到同一个零距离 `DragGesture` 路由；达到既有 12pt policy 后立即由同一 callback claim pan/area lease，避免独立 `SpatialTapGesture` 与第二个 12pt drag 的 callback 竞态造成拖动死区。双指框选/捏合、长按上下文、TouchSequenceOwner、命令语义、44pt、生产、模型、战斗和 Web 版保持；这是 BattlefieldView 输入层改动。实现 commit `0dae3955d858aee68bfbcb1f868dce2248c48f24` 对应 Actions run `33298412664` / attempt `1` / job `99221942112` 的 artifact `rustwar-ci-v1.2-main-0dae395-run33298412664-attempt1`（ID `9728269520`，GitHub digest `sha256:e092e1e24b0ca51a603d5684884721e49fe3e4d7a7c1cdb8ed6763020406c188`，网页大小约 1.32 MB，下载目录约 1.7M）已由 Agent C 下载到 `/private/tmp/rustwar-c-review-33298412664/` 并核对；manifest 与完整 SHA、branch、run/attempt 和固定工具链完全匹配。JUnit `8/0/1`、Core `344 tests`、`BattlefieldView.swift` 双架构编译、Xcode list/build、production/combat 双场景启动、横屏归一化和双 PNG probe 全成功；既有 headless-browser regression 仍为唯一 skip。Home SHA-256 `8ca15f5341017c1760863c38d64c70bb5b86c789907cd2371c44ce2e5ff2513f`，Combat `b6eb46da36d6363de916dce479edb7de4eaa6f01b1c321ba006423656e975021`，与 v2.82 一致。Agent C 人工确认单指路由改动没有静态 UI、模型、武器、terminal/impact、Tactical Map 或状态栏回退；真实设备 callback 顺序与手感仍属于证据边界。

v2.82（云端验收通过）：原生 iOS 普通 compact command dock 的 Battlefield interaction hint 现在使用按状态派生的短可见文案，让“点敌人攻击 / 点空地 Attack-Move / 拖动框选 / 点位置”不再用长句把 Quick Orders 推到首屏之外；VoiceOver、regular/accessibility Dynamic Type 继续读取完整触控说明，生产建筑、命令 action、44pt、Core、战斗和 Web 版不变。本轮借鉴 Rusted Warfare 对快速发令、multi-touch、unit groups 和 strategic zoom 的公开产品定位，属于 SwiftUI presentation-only 改动。实现 commit `243723bc24aa161cda46a17c50cfb41294b7a0d6` 对应 Actions run `33296047909` / attempt `1` / job `99215769992` 的 artifact `rustwar-ci-v1.2-main-243723b-run33296047909-attempt1`（ID `9727507874`，GitHub digest `sha256:da60a08ef50d995983f5361d3015aa8a08f06f98b6fa61673f2afc3d9dbed392`，网页大小 1.32 MB，下载目录约 1.7M）已由 Agent C 下载到 `/private/tmp/rustwar-c-review-33296047909/` 并核对；manifest 与完整 SHA、branch、run/attempt 和固定工具链完全匹配。JUnit `8/0/1`、Core `344 tests`、SwiftUI 修改文件双架构编译、Xcode list/build、production/combat 双场景启动、横屏归一化和双 PNG probe 全成功；既有 headless-browser regression 仍为唯一 skip。Home SHA-256 `8ca15f5341017c1760863c38d64c70bb5b86c789907cd2371c44ce2e5ff2513f`，Combat `b6eb46da36d6363de916dce479edb7de4eaa6f01b1c321ba006423656e975021`。Agent C 人工确认紧凑提示、Quick Orders/Commands、生产 HUD、单位模型、武器与 terminal/impact、Tactical Map 和状态栏无静态回退；触控 callback 顺序、VoiceOver、Dynamic Type、滚动和真机手感仍属于证据边界。

v2.81（云端验收通过）：原生 iOS 单位主体的中间装甲面增加低饱和队伍色偏移：玩家为绿灰、敌方为红灰；原有深色钢灰层级、八类单位的几何模型、炮塔/发射器、v2.78 hull 尾缘单/双阵营标记、选择/HP/战斗特效保持。该轮只改变 `BattlefieldScene.unitBody` 的 presentation，不改变建筑、武器后坐、Core、命令、输入、生产、AI、存档、HUD、Tactical Map 或 Web 版。实现 commit `8c397a4b937ad44892ebc8e4183702f1f478382a`、格式修复 commit `e89fcee6499cd9377ce970bfbea7feb4cc80b188` 对应 Actions run `33294202486` / attempt `1` / job `99210960672` 的 artifact `rustwar-ci-v1.2-main-e89fcee-run33294202486-attempt1`（ID `9727019052`，1.7M）已由 Agent C 下载并核对；GitHub API 未提供该 artifact 的 digest。JUnit `8/0/1`、Core `344 tests`、双架构 build、双场景启动/横屏归一化和双 PNG probe 全成功；Home `8ca15f5341017c1760863c38d64c70bb5b86c789907cd2371c44ce2e5ff2513f`，Combat `d6e042d8b9d7a46bcf4248fb20f94ca52b15b93f1bc892cb188a36d809658fda`，相对 v2.80 均改变。

v2.80（云端验收通过）：原生 iOS compact producer dock 的固定 header 已显示 `Production / Land Factory / T2 / 1.25x`，因此普通非辅助功能字号下不再在 Production section 紧接着重复渲染第二个 `Production` section title，management rail、NOW / QUEUE / UPGRADE、Factory Tech、生产卡和队列更早进入首屏。regular、compact 下的辅助功能 Dynamic Type、无 producer focus 的 section、VoiceOver（含 `.isHeader` heading）、44pt、快捷键、生产 action、Core、存档和 Web 版保持；本轮只改变 SwiftUI presentation gate。修复实现 commit `86165b7210224f8251e6fc753ada428824cfcea4` 对应 Actions run `33292609260` / attempt `1` / job `99206768465` 的 artifact `rustwar-ci-v1.2-main-86165b7-run33292609260-attempt1`（ID `9726513313`，digest `sha256:70951d4e15d1c2b09f57e513cf674dfbd2a849ff416ae9b721c68f711dff8830`）已由 Agent C 下载并核对：JUnit `8/0/1`、Core `344 tests`、双架构 build、双场景启动/横屏归一化和双 PNG probe 全成功；Home `f7ce884d...`，Combat `c4557ffe...` 与 v2.79 一致。

v2.79（云端验收通过）：原生 iOS 主战场增加 fresh-touch handoff 的 terminal barrier。当 owner 仍持有 primary、同一 Spatial frame 同时出现该 primary 的 ended/cancelled 与未知 active touch 时，TouchSequenceOwner 原子记录旧 terminal、暂缓 fresh ID，不把新触点判成 replacement 或加入 quarantine，等待无歧义 active frame 后再播种新 sequence；若 terminal 是 cancelled，则同时推进旧 owner 的 cancellation epoch 使旧 lease 失效。owner 已进入 terminal-possible 后，未知 terminal 仍由 fresh-seed barrier 阻塞。实现 commit `a1b5a3935a529e120989265ea7d42476bf9865b2` 对应 Actions run `33291279372` / attempt `1` / job `99203215236` 的 artifact `rustwar-ci-v1.2-main-a1b5a39-run33291279372-attempt1` 已由 Agent C 下载并核对：JUnit `8/0/1`、Core `344 tests`、双架构源码编译、双场景启动/横屏归一化和双 PNG probe 全成功；Home `166db0cd...`、Combat `c4557ffe...` 与 v2.78 一致。已 quarantine 的旧 ID 仍可正常交接；不声称能绝对区分所有 SwiftUI 旧触点回调。
