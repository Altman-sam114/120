public enum GameDefinitions {
    public static let units: [UnitType: UnitDefinition] = [
        ...
        .heavyTank: UnitDefinition(type: .heavyTank, name: "Heavy Tank", icon: "HT", hitPoints: 520, radius: 19, speed: 48, vision: 300, supply: 4, metalCost: 420, buildTime: 14, attackRange: 205, damage: 82, reloadTime: 1.75, requiredProducerUpgradeLevel: 2),
    ...
    public static let buildings: [BuildingType: BuildingDefinition] = [
        ...
        .landFactory: BuildingDefinition(
            ...
            supply: 8,
            vision: 310,
            produces: [.scout, .tank, .hover, .artillery, .aaTank, .heavyTank],
            upgrades: [
                BuildingUpgradeDefinition(
                    level: 2,
    ...
    public static func productionUnits(for producer: BuildingSnapshot) -> [UnitType] {
        building(producer.type).produces.filter {
            unit($0).requiredProducerUpgradeLevel <= producer.upgradeLevel
        }
    }

    public static func canProduce(_ unitType: UnitType, at producer: BuildingSnapshot) -> Bool {
        productionUnits(for: producer).contains(unitType)
    }
}
